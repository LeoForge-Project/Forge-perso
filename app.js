...
const done = data.habits.filter(h=>h.done);
document.getElementById(‘view-habits’).innerHTML = `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"> <div class="title">Habitudes</div> <div style="display:flex;align-items:center;gap:8px"> <span class="tag tag-a">${done.length}/${data.habits.length}</span> <button onclick="openHabitModal(null)" class="btn" style="padding:8px 14px;font-size:11px">+ Ajouter</button> </div> </div> <div style="display:flex;flex-direction:column;gap:7px" id="habits-list"> ${data.habits.map(h => habitHTML(h)).join('')} </div> ${!data.habits.length ?`<div style="text-align:center;padding:60px;color:var(--muted)">Aucune habitude — clique + Ajouter</div>` : ''}`;
}
...
document.getElementById(‘habit-modal-body’).innerHTML = ` <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px"> <div class="title" style="font-size:17px">${e?'Modifier':'Nouvelle habitude'}</div> <button onclick="closeModal('habit-modal')" style="color:var(--muted);font-size:18px;line-height:1">✕</button> </div> <div style="display:flex;flex-direction:column;gap:13px"> <div><div class="label" style="margin-bottom:5px">Nom</div><input id="hm-name" class="input" placeholder="Ex: Méditation 10 min" value="${e?.name||''}"></div> <div style="display:grid;grid-template-columns:80px 1fr;gap:10px"> <div><div class="label" style="margin-bottom:5px">Emoji</div><input id="hm-emoji" class="input" placeholder="🔥" value="${e?.emoji||'📌'}" style="text-align:center"></div> <div><div class="label" style="margin-bottom:5px">XP (+${e?.xp||40})</div><div class="slider-row"><input type="range" id="hm-xp" min="10" max="100" step="5" value="${e?.xp||40}" oninput="document.getElementById('hm-xp-v').textContent=this.value+' XP'"><span id="hm-xp-v" class="sval">${e?.xp||40} XP</span></div></div> </div> <div><div class="label" style="margin-bottom:6px">Pilier</div><div class="chips">${pillars.map(([v,l])=>`<button class="chip ${(e?.pillar||'discipline')===v?'on':''}" onclick="selectChip(this,'hm-pillar')" data-val="${v}">${l}</button>`).join('')}</div></div> <button class="btn" onclick="saveHabit()" style="margin-top:4px">✓ Enregistrer</button> </div>`;
...
return `<div class="cal-dhead ${isToday?'today':''}"> <div style="font-family:'JetBrains Mono',monospace;font-size:8px;letter-spacing:2px;color:${isToday?'#C8FF00':'var(--muted)'}">${d}</div> <div style="font-family:'JetBrains Mono',monospace;font-size:14px;font-weight:700;color:${isToday?'#C8FF00':'var(--white)'}">${dt.getDate()}</div> </div>`;
}).join(’’);
...
document.getElementById(‘view-calendar’).innerHTML = ` <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px"> <div class="title">Calendrier</div> <button onclick="go('scanner')" class="btn-ghost" style="font-size:11px">📷 Scanner EDT</button> </div> <div class="cal-head"><div></div>${dayHeads}</div> <div class="cal-body">${rows}</div>`;
}
...
document.getElementById(‘scan-results’).innerHTML = _scanSlots.map(s=>` <div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--card);border:1px solid var(--border);border-radius:6px"> <span style="font-family:'JetBrains Mono',monospace;font-size:9px;color:#C8FF00;width:80px;flex-shrink:0">${s.start}–${s.end}</span> <div style="flex:1;min-width:0"><div style="font-size:12px;font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${s.name}</div>${s.room?`<div style="font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--muted)">${s.room}</div>`:''}</div> <span style="font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--muted);flex-shrink:0">${s.day}</span> </div>`).join(’’);
...
if (!list) return;
list.innerHTML = chats.length ? chats.map(c=>` <div class="chat-item ${c.id===currentChatId?'on':''}" onclick="loadChat('${c.id}')"> <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:4px"> <div class="chat-item-title">${c.title}</div> <button class="chat-item-del" onclick="event.stopPropagation();delChat('${c.id}')">✕</button> </div> <div class="chat-item-date">${fmtDate(c.date)} · ${(c.mode||'business').toUpperCase()}</div> </div>`).join(’’) : `<div style="padding:16px;text-align:center;color:var(--muted);font-size:12px">Aucune conversation</div>`;
}
...
const container = document.getElementById(‘chat-messages’);
if (!container) return;
container.innerHTML = ` <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;opacity:.5"> <div style="font-size:32px">✦</div> <div style="font-family:'Syne',sans-serif;font-weight:700;font-size:14px;letter-spacing:1px">Conseiller IA</div> <div style="font-size:12px;color:var(--muted);text-align:center;max-width:240px">Business · TikTok sans visage · Coach<br>Je suis là pour t'aider.</div> </div>`;
}
